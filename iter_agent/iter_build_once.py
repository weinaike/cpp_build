...
2. **Environment-First Strategy**:
   ...
   - **Minimize source code modifications** to preserve codebase integrity'''
    ...
    try:
        ...
        import traceback
        print(traceback.format_exc())
        proposal = "No proposal available due to error."
    print(f"✅ 建议获取完成")
...
def start_docker(project):
    ...
    try:
        ...
        if result.stdout.strip():
            ...
            # 等待容器完全启动
            time.sleep(2)
        # 更新容器资源限制，同时设置 memory-swap 避免冲突
        subprocess.run(["docker", "update", "--cpus", "8", "--memory", "16G", "--memory-swap", "16G", "--memory-reservation", "8G", container_name], check=True)
        return True, should_stop_container
